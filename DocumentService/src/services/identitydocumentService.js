...
exports.getAllIdentityDocument = async (consumer, category) => {
    const idocs = await IdentityDocument.find({ consumer: consumer, category: category })

    // Iterate through each document and add URL
    await Promise.all(idocs.map(async (idoc) => {
 ...
 */
exports.getIdentityDocumentByDoc_type = async (consumer, cat, doctype) => {
    const idoc = await IdentityDocument.findOne({ consumer: consumer, category: cat, doctype: doctype })
    if (!idoc) {
...
exports.addIdentityDocument = async (consumer, payload, file) => {
    ...
    payload.expiration_date = moment(expiration_date, 'DD-MM-YYYY').format('YYYY-MM-DD')
    ...
    if (await IdentityDocument.isDocumentExist(consumer, category, doctype)) {
        ...
        await this.updateIdentityDocumentByDoc_type(consumer, category, doctype, payload, file)
    } else {
        ...
        try {
            ...
            return { message: 'Identity document created successfully under category', data: createdDocument, url: url }
        } catch (uploadError) {
            ...
            throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to upload file to cloud storage')
        }
    }
 ...
 */
exports.updateIdentityDocumentByDoc_type = async (consumer, category, doctype, payload, file) => {
    // Retrieve the original document
    const originalDocument = await IdentityDocument.findOne({ consumer: consumer, category: category, doctype: doctype })
    if (!originalDocument) {
    ...
    // Format expiration date 
    payload.expiration_date = moment(payload.expiration_date, 'DD-MM-YYYY').format('YYYY-MM-DD')
    ...
    if (file) {
        ...
        try {
            ...
            throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Failed to update file in cloud storage')
        }
    }
    return {
...
exports.deleteIdentityDocumentByDoc_type = async (consumer, cat, doctype) => {
    ...
    const idoc = await IdentityDocument.findOne({ consumer, category: cat, doctype: doctype })
